    ...
    <div className="space-y-4">
      ...
      <div className="overflow-auto relative max-h-[80vh]">
        <div className={`grid text-sm min-w-max`} style={{
          ...
          {/* Time column header */}
          <div className="sticky top-0 left-0 z-30 p-2 text-sm font-semibold text-center bg-white border-b border-r text-black">時間</div>
          ...
          {timeLabels.map(time => (
            <React.Fragment key={time}>
              {/* Time label row */}
              <div className="sticky left-0 z-20 p-2 text-sm text-center bg-white border-b border-r whitespace-nowrap font-medium text-black">{time}</div>
              {/* TimeSlot components */}
              {days.map(day => {
